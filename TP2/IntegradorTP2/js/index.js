...
document.addEventListener("DOMContentLoaded", async function() {
  ...
  // PAR 2 G2
  image = document.createElement('img');
  image.src = 'images/f1g2.png';
  await image.onload;
  f1g2 = image;

  image = document.createElement('img');
  image.src = 'images/f2g2.png';
  await image.onload;
  f2g2 = image;

  // PAR 3 G3
  image = document.createElement('img');
  image.src = 'images/f1g3.png';
  await image.onload;
  f1g3 = image;

  image = document.createElement('img');
  image.src = 'images/f2g3.jpg';
  await image.onload;
  f2g3 = image;
  ...
  canvas.addEventListener('mouseup', function(evt) {
    if (juego.juegoIniciado()){
      ...
      if ((ficha_seleccion != null) && punteroSobreTablero(evt)){
        ...
        // }
        // else{
          columna = tablero.getColumnaFicha(ficha_seleccion);
          //inserta la ficha en caso de tener disponibilidad. devuelve boolean y cambia el turno de jugador
          if ( columna != null && juego.insertarFicha(ficha_seleccion,columna)){
            juego.cambiarTurno();
          } else {
            tablero.draw();
            ficha_seleccion.drawPosicionOriginal();
      ...
      juego.redibujarFichas(null,1);
      juego.redibujarFichas(null,2);
    }
  }, false);
  /**
...
// Lectura de la posicion del puntero del mouse
...
function reiniciarJuego(){
  ...
  asignarFondoFichas(grupo_seleccionado);
...
function setearTiempoJuego(){
  juego.setTiempoDeJuego(document.getElementById('input_tiempo_juego').value % 60);
}
...
function asignarFondoFichas(grupo){
  if (!juego.juegoIniciado()){
    if (grupo == 1){
      juego.asignarFondoFichas(f1g1,f2g1);
      grupo_seleccionado =1;
    }
    if (grupo == 2){
      juego.asignarFondoFichas(f1g2,f2g2);
      grupo_seleccionado =2;
    }
    if (grupo == 3){
      juego.asignarFondoFichas(f1g3,f2g3);
      grupo_seleccionado =3;
    }
  }
}